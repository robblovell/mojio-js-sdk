...
(function() {
  var Http, MojioClient, Observer, SignalR,
  ...
  module.exports = MojioClient = (function() {
    ...
    function MojioClient(options) {
      ...
      this.observer = __bind(this.observer, this);
      ...
      this.token = null;
      this.signalr = new SignalR.client("http://" + this.options.hostname + ":" + this.options.port + "/v1/signalr", ['ObserverHub']);
    }
    ...
    */
    ...
    MojioClient.prototype.observer_callbacks = {};

    MojioClient.prototype.observer_registry = function(entity) {
    ...
    MojioClient.prototype.observer = function(object, subject, observer_callback, callback) {
      var observer,
        _this = this;
      if (subject == null) {
        subject = null;
      ...
      return this.request({
        ...
        hub = _this.getHub();
        if (_this.observer_callbacks[result.SubjectId] == null) {
          _this.observer_callbacks[result.SubjectId] = [];
        }
        _this.observer_callbacks[result.SubjectId].push(observer_callback);
        hub.invoke('Subscribe', result._id);
        return callback(null, observer);
      });
    };

    MojioClient.prototype.unobserve = function(observer, observer_callback) {
      var callback, temp, _i, _len, _ref;
      if (observer_callback == null) {
        observer_callback = null;
      }
      if (observer_callback === null) {
        this.observer_callbacks[observer.SubjectId] = [];
      } else {
        temp = [];
        _ref = this.observer_callbacks[observer.SubjectId];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          callback = _ref[_i];
          if (callback !== observer_callback) {
            temp[observer.SubjectId].push(callback);
          }
        }
        this.observer_callbacks[observer.SubjectId] = temp;
      }
      if (this.observer_callbacks[observer.SubjectId].length === 0) {
    ...
    */
    ...
    MojioClient.prototype.getHub = function() {
      ...
      this.hub = this.signalr.hub('ObserverHub');
