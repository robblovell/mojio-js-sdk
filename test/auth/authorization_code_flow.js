// Generated by CoffeeScript 1.9.3
(function() {
  var MojioAuthSDK, MojioSDK, app, authorization_uri, client_id, client_secret, express, implicit, oauth2, redirect_uri, redirect_uri2, sdk;

  express = require('express');

  MojioSDK = require('../../src/nodejs/sdk/MojioSDK');

  MojioAuthSDK = require('../../src/nodejs/sdk/MojioAuthSDK');

  app = express();

  client_id = '5f81657f-47f6-4d86-8213-5c01c1f3a243';

  client_secret = 'fcca6c06-3d30-488e-947b-a6291e39ff3c';

  oauth2 = require('simple-oauth2')({
    clientID: client_id,
    clientSecret: client_secret,
    site: 'https://staging-accounts.moj.io',
    tokenPath: '/oauth2/token',
    authorizationPath: '/oauth2/authorize'
  });

  redirect_uri = 'http://localhost:3000/callback';

  redirect_uri2 = 'http://localhost:3000/callback';

  authorization_uri = oauth2.authCode.authorizeURL({
    client_id: '5f81657f-47f6-4d86-8213-5c01c1f3a243',
    redirect_uri: redirect_uri,
    scope: ['full']
  });

  sdk = new MojioSDK({
    sdk: MojioAuthSDK,
    client_id: client_id,
    client_secret: client_secret,
    site: 'https://staging-accounts.moj.io',
    tokenPath: '/oauth2/token',
    authorizationPath: '/oauth2/authorize',
    test: true
  });

  implicit = false;

  app.get('/authCode', function(req, res) {
    implicit = false;
    console.log("res:" + res);
    return sdk.authorize(redirect_uri).scope(['full']).redirect(res);
  });

  app.get('/authImplicit', function(req, res) {
    implicit = true;
    console.log("res:" + res);
    return sdk.authorize(redirect_uri, implicit).scope(['full']).redirect(res);
  });

  app.get('/logout', function(req, res) {
    var loggedOut;
    console.log("res:" + res);
    loggedOut = function(error, result) {
      return console.log("logged out");
    };
    return sdk.unauthorize("http://localhost:3000/callback", implicit).login().redirect(res);
  });

  app.get('/consent', function(req, res) {
    var loggedOut;
    console.log("res:" + res);
    loggedOut = function(error, result) {
      return console.log("Removed Consent");
    };
    return sdk.unauthorize("http://localhost:3000/callback", implicit).consent().redirect(res);
  });

  app.get('/unauth', function(req, res) {
    var loggedOut;
    console.log("res:" + res);
    loggedOut = function(error, result) {
      return console.log("logged out and removed consent");
    };
    return sdk.unauthorize("http://localhost:3000/callback", implicit).login().consent().redirect(res);
  });

  app.get('/callback', function(req, res) {
    var code, saveToken;
    code = req.query.code;
    console.log('/callback');
    saveToken = function(error, result) {
      var token;
      if (error) {
        console.log('Access Token Error', JSON.stringify(error.content) + "  message:" + error.statusMessage + "  url:" + sdk.url());
        return res.send('Access Token Error: ' + JSON.stringify(error.content) + "  message:" + error.statusMessage + "  url:" + sdk.url());
      } else {
        token = oauth2.accessToken.create(result);
        console.log("Token:" + JSON.stringify(token));
        return res.send('World: <br><a href="/unauth">Unauthorize Mojio</a><br><a href="/logout">Log out of Mojio</a><br><a href="/consent">Remove consent from Mojio</a>');
      }
    };
    return sdk.token().parse(req, redirect_uri).callback(saveToken);
  });

  app.get('/', function(req, res) {
    return res.send('Hello<br><a href="/authCode">Log in with Mojio: Authorization Code Flow</a><br><a href="/authImplicit">Log in with Mojio: Implicit Flow</a>');
  });

  app.listen(3000);

  console.log('Express server started on port 3000');

}).call(this);

//# sourceMappingURL=authorization_code_flow.js.map
