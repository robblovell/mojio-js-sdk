...
#        styles: [MojioPromiseStyle]
    ...
    it 'can GET a resource or list of resources, secondary resource, or history by ids or no query', (testComplete) ->
        setupTokenNock = () ->
...
#        @.timeout(5000)
        docall = (item, finishedOne) ->
#            if item.primary == 'Trips' and item.secondary == 'History' and item.tertiary == 'Locations'
#                finish = done
#            else
#                finish = (() ->)
            execute( (cb) ->
                if (item.tertiary?)
                    ...
                    sdk.get()[item.primary](item.pid)[item.secondary]()[item.tertiary]()
                    .callback((error, result) ->
                        testErrorResult(error, result)
                        cb(null, result)
                    )
                    ...
                    if item.sid?
                        ...
                        sdk.get()[item.primary](item.pid)[item.secondary](item.sid)
                        .callback((error, result) ->
                            testErrorResult(error, result)
                            cb(null, result)
                        )
                        ...
                        sdk.get()[item.primary](item.pid)[item.secondary]()
                        .callback((error, result) ->
                            testErrorResult(error, result)
                            cb(null, result)
                        )
        ...
        calls = []
        calls.push((callback) ->
            docall({primary:'me'}, (error, result) ->
        ...
        for entity in entities
            calls.push((callback) ->
                docall({primary:entity}, (error, result) ->
                    console.log(""+entity+": done"+result)
                    callback(error,result)
                )
            )
            calls.push((callback) ->
                docall({primary:entity, pid: i++}, (error, result) ->
                    console.log(""+entity+"/"+": done"+result)
                    callback(error,result)
                )
            )
            for secondaryEntity in secondaries
                calls.push((callback) ->
                    docall({primary:entity, pid: i++, secondary: secondaryEntity}, (error, result) ->
                        console.log(""+entity+"/"+": done"+result)
                        callback(error,result)
                    )
                )
                # TODO: primary/id/secondary/id fix.
#                calls.push((callback) ->
#                    execute({primary:entity, pid: i++, secondary: secondaryEntity, sid: i++}, (error, result) ->
#                        console.log(""+entity+"/"+": done"+result)
#                        callback(error,result)
#                    )
#                )
        ...
        for entity in entities2
            for secondaryEntity in secondaries2
                for tertiaryEntity in tertiary2
                    calls.push((callback) ->
                        docall({
                            primary:entity,pid: i++,secondary: secondaryEntity,sid: null,
                            tertiary: tertiaryEntity, tid: null}, (error, result) ->
                            console.log(""+entity+"/"+": done"+result)
                            callback(error,result)
                        )
                    )
