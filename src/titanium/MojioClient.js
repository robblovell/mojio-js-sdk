(function() {
    var Http, MojioClient, SignalR;
    Http = require("./HttpTitaniumWrapper");
    SignalR = "tbd";
    var appname = "myfirstappeceleraterapp";
    var accessToken = null;
    Ti.API.info('DON"T LOAD SingnalR!!');
    module.exports = MojioClient = function() {
        function MojioClient(options) {
            var base, base1, base2, base3, base4, base5, base6, base7;
            this.options = options;
            null == this.options && (this.options = {
                hostname: this.defaults.hostname,
                port: this.defaults.port,
                version: this.defaults.version,
                scheme: this.defaults.scheme,
                live: this.defaults.live
            });
            null == (base = this.options).hostname && (base.hostname = defaults.hostname);
            null == (base1 = this.options).port && (base1.port = defaults.port);
            null == (base2 = this.options).version && (base2.version = defaults.version);
            null == (base3 = this.options).scheme && (base3.scheme = defaults.scheme);
            null == (base4 = this.options).signalr_port && (base4.signalr_port = defaults.signalr_port);
            null == (base5 = this.options).signalr_scheme && (base5.signalr_scheme = defaults.signalr_scheme);
            null == (base6 = this.options).signalr_hub && (base6.signalr_hub = defaults.signalr_hub);
            this.options.application = this.options.application;
            this.options.secret = this.options.secret;
            this.options.observerTransport = "SingalR";
            this.conn = null;
            this.hub = null;
            this.connStatus = null;
            this.auth_token = null;
            this.access_token = null;
            null == (base7 = this.options).tokenRequester && (base7.tokenRequester = function() {
                return document.location.hash.match(/access_token=([0-9a-f-]{36})/);
            });
        }
        var App, Event, Mojio, Observer, Product, Subscription, Trip, User, Vehicle, defaults, mojio_models, authorized;
        defaults = {
            hostname: "api.moj.io",
            port: "443",
            version: "v1",
            scheme: "https",
            signalr_scheme: "https",
            signalr_port: "443",
            signalr_hub: "ObserverHub",
            live: true
        };
        authorized = false;
        MojioClient.prototype.getResults = function(type, results) {
            var arrlength, i, j, len, len1, objects, ref, result;
            Ti.API.info("test getResults, results is" + results);
            objects = [];
            if (results instanceof Array) {
                arrlength = results.length;
                Ti.API.info("debugging, result is Array?" + arrlength);
                for (i = 0, len = results.length; len > i; i++) {
                    Ti.API.info("data is array!");
                    result = results[i];
                    objects.push(new type(result));
                }
            } else if (results.Data instanceof Array) {
                arrlength = results.Data.length;
                Ti.API.info("debugging, result data is Array?" + arrlength);
                ref = results.Data;
                for (j = 0, len1 = ref.length; len1 > j; j++) {
                    result = ref[j];
                    Ti.API.info("result is : " + result);
                    Ti.API.info("ref is : " + JSON.stringify(result));
                    LastLocation = result.LastLocation;
                    Ti.API.info("LastLocation : " + LastLocation);
                    Lat = LastLocation.Lat;
                    Ti.API.info("Lat : " + Lat);
                    objects.push(new type(result));
                }
            } else if (null !== result.Data) {
                Ti.API.info("debugging, result is Data?");
                objects.push(new type(result.Data));
            } else {
                Ti.API.info("result is result");
                objects.push(new type(result));
            }
            return objects;
        };
        MojioClient._makeParameters = function(params) {
            var property, query, value;
            0 === params.length && "";
            query = "?";
            for (property in params) {
                value = params[property];
                query += encodeURIComponent(property) + "=" + encodeURIComponent(value) + "&";
            }
            return query.slice(0, -1);
        };
        MojioClient.prototype.getPath = function(resource, id, action, key) {
            if (key && id && action && "" !== id && "" !== action) return "/" + encodeURIComponent(resource) + "/" + encodeURIComponent(id) + "/" + encodeURIComponent(action) + "/" + encodeURIComponent(key);
            if (id && action && "" !== id && "" !== action) return "/" + encodeURIComponent(resource) + "/" + encodeURIComponent(id) + "/" + encodeURIComponent(action);
            if (id && "" !== id) return "/" + encodeURIComponent(resource) + "/" + encodeURIComponent(id);
            if (action && "" !== action) return "/" + encodeURIComponent(resource) + "/" + encodeURIComponent(action);
            return "/" + encodeURIComponent(resource);
        };
        MojioClient.prototype.dataByMethod = function(data, method) {
            switch (method.toUpperCase()) {
              case "POST":
                return this.stringify(data);

              default:
                return data;
            }
        };
        MojioClient.prototype.stringify = function(data) {
            return JSON.stringify(data);
        };
        MojioClient.prototype.request = function(request, callback) {
            var http, parts;
            parts = {
                hostname: this.options.hostname,
                host: this.options.hostname,
                port: this.options.port,
                scheme: this.options.scheme,
                path: "/" + this.options.version,
                method: request.method,
                withCredentials: false
            };
            parts.path = "/" + this.options.version + this.getPath(request.resource, request.id, request.action, request.key);
            null != request.parameters && Object.keys(request.parameters).length > 0 && (parts.path += MojioClient._makeParameters(request.parameters));
            parts.headers = {};
            null != this.getTokenId() && (parts.headers["MojioAPIToken"] = this.getTokenId());
            null != request.headers && (parts.headers += request.headers);
            parts.headers["Content-Type"] = "application/json";
            null != request.body && (parts.body = request.body);
            Ti.API.info("Z Yang, debugging: " + parts.headers);
            http = new Http();
            return http.request(parts, callback);
        };
        MojioClient.prototype.login_resource = "Login";
        MojioClient.prototype.authorize = function(redirect_url, scope) {
            if (null === accessToken) {
                var parts, url;
                null == scope && (scope = "full");
                parts = {
                    hostname: this.options.hostname,
                    host: this.options.hostname,
                    port: this.options.port,
                    scheme: this.options.scheme,
                    path: this.options.live ? "/OAuth2/authorize" : "/OAuth2Sandbox/authorize",
                    method: "Get",
                    withCredentials: false
                };
                parts.path += "?response_type=token";
                parts.path += "&client_id=" + this.options.application;
                parts.path += "&redirect_uri=" + redirect_url;
                parts.path += "&scope=" + scope;
                parts.headers = {};
                null != this.getTokenId() && (parts.headers["MojioAPIToken"] = this.getTokenId());
                parts.headers["Content-Type"] = "application/json";
                url = parts.scheme + "://" + parts.host + ":" + parts.port + parts.path;
                var webview = Titanium.UI.createWebView();
                Ti.API.info("webview");
                webview.setUrl(url);
                webview.addEventListener("load", function(e) {
                    if (0 === e.url.indexOf(appname)) {
                        webview.setVisible(false);
                        e.bubble = false;
                        webview.stopLoading();
                        window.remove(webview);
                        webview.hide();
                        var tokenIndex = e.url.indexOf("token");
                        accessToken = e.url.substring(tokenIndex + 6, tokenIndex + 42);
                        Ti.API.info("obtained token: " + accessToken);
                        authorized = true;
                    }
                });
                return webview;
            }
        };
        MojioClient.prototype.isauthorized = function() {
            return authorized;
        };
        MojioClient.prototype.token = function(callback) {
            var match, token;
            this.user = null;
            match = accessToken;
            token = accessToken;
            Ti.API.info("token", token);
            return token ? this.request({
                method: "GET",
                resource: this.login_resource,
                id: token
            }, function(_this) {
                return function(error) {
                    var e = error.error;
                    if (e) return callback(e, null);
                    _this.auth_token = JSON.parse(this.responseText);
                    return callback(e, _this.auth_token);
                };
            }(this)) : callback("token for authorization not found.", null);
        };
        MojioClient.prototype.showtoken = function() {
            null == accessToken && alert("haven't got the token yet!");
            return accessToken;
        };
        MojioClient.prototype.unauthorize = function(redirect_url) {
            var parts, url;
            parts = {
                hostname: this.options.hostname,
                host: this.options.hostname,
                port: this.options.port,
                scheme: this.options.scheme,
                path: "/account/logout",
                method: "Get",
                withCredentials: false
            };
            parts.path += "?Guid=" + this.getTokenId();
            parts.path += "&client_id=" + this.options.application;
            parts.path += "&redirect_uri=" + redirect_url;
            parts.headers = {};
            null != this.getTokenId() && (parts.headers["MojioAPIToken"] = this.getTokenId());
            parts.headers["Content-Type"] = "application/json";
            url = parts.scheme + "://" + parts.host + ":" + parts.port + parts.path;
            var webview = Titanium.UI.createWebView();
            webview.setUrl(url);
            return webview;
        };
        MojioClient.prototype._login = function(username, password, callback) {
            return this.request({
                method: "POST",
                resource: this.login_resource,
                id: this.options.application,
                parameters: {
                    userOrEmail: username,
                    password: password,
                    secretKey: this.options.secret
                }
            }, callback);
        };
        MojioClient.prototype.login = function(username, password, callback) {
            return this._login(username, password, function(_this) {
                return function(error, result) {
                    null != result && (_this.auth_token = result);
                    return callback(error, result);
                };
            }(this));
        };
        MojioClient.prototype._logout = function(callback) {
            return this.request({
                method: "DELETE",
                resource: this.login_resource,
                id: "undefined" != typeof mojio_token && null !== mojio_token ? mojio_token : this.getTokenId()
            }, callback);
        };
        MojioClient.prototype.logout = function(callback) {
            return this._logout(function(_this) {
                return function(error, result) {
                    _this.auth_token = null;
                    return callback(error, result);
                };
            }(this));
        };
        mojio_models = {};
        App = require("../models/App");
        mojio_models["App"] = App;
        Mojio = require("../models/Mojio");
        mojio_models["Mojio"] = Mojio;
        Trip = require("../models/Trip");
        mojio_models["Trip"] = Trip;
        User = require("../models/User");
        mojio_models["User"] = User;
        Vehicle = require("../models/Vehicle");
        mojio_models["Vehicle"] = Vehicle;
        Product = require("../models/Product");
        mojio_models["Product"] = Product;
        Subscription = require("../models/Subscription");
        mojio_models["Subscription"] = Subscription;
        Event = require("../models/Event");
        mojio_models["Event"] = Event;
        Observer = require("../models/Observer");
        mojio_models["Observer"] = Observer;
        MojioClient.prototype.model = function(type, json) {
            var data, i, len, object, ref;
            null == json && (json = null);
            if (null === json) return mojio_models[type];
            if (null != json.Data && json.Data instanceof Array) {
                object = json;
                object.Objects = new Array();
                ref = json.Data;
                for (i = 0, len = ref.length; len > i; i++) {
                    data = ref[i];
                    object.Objects.push(new mojio_models[type](data));
                }
            } else object = new mojio_models[type](null != json.Data ? json.Data : json);
            object._client = this;
            return object;
        };
        MojioClient.prototype.query = function(model, parameters, callback) {
            var property, query_criteria, ref, value;
            if (parameters instanceof Object) {
                if (parameters.criteria instanceof Object) {
                    query_criteria = "";
                    ref = parameters.criteria;
                    for (property in ref) {
                        value = ref[property];
                        query_criteria += property + "=" + value + ";";
                    }
                    parameters.criteria = query_criteria;
                }
                return this.request({
                    method: "GET",
                    resource: model.resource(),
                    parameters: parameters
                }, function(_this) {
                    return function(error, result) {
                        result = JSON.parse(this.responseText);
                        return callback(error, _this.model(model.model(), result));
                    };
                }(this));
            }
            return "string" == typeof parameters ? this.request({
                method: "GET",
                resource: model.resource(),
                parameters: {
                    id: parameters
                }
            }, function(_this) {
                Ti.API.info("mojioclient!");
                return function(error, result) {
                    result = JSON.parse(this.responseText);
                    return callback(error, _this.model(model.model(), result));
                };
            }(this)) : callback("criteria given is not in understood format, string or object.", null);
        };
        MojioClient.prototype.get = function(model, criteria, callback) {
            return this.query(model, criteria, callback);
        };
        MojioClient.prototype.save = function(model, callback) {
            return this.request({
                method: "PUT",
                resource: model.resource(),
                body: model.stringify(),
                parameters: {
                    id: model._id
                }
            }, callback);
        };
        MojioClient.prototype.put = function(model, callback) {
            return this.save(model, callback);
        };
        MojioClient.prototype.create = function(model, callback) {
            return this.request({
                method: "POST",
                resource: model.resource(),
                body: model.stringify()
            }, callback);
        };
        MojioClient.prototype.post = function(model, callback) {
            return this.create(model, callback);
        };
        MojioClient.prototype["delete"] = function(model, callback) {
            return this.request({
                method: "DELETE",
                resource: model.resource(),
                parameters: {
                    id: model._id
                }
            }, callback);
        };
        MojioClient.prototype._schema = function(callback) {
            return this.request({
                method: "GET",
                resource: "Schema"
            }, callback);
        };
        MojioClient.prototype.schema = function(callback) {
            return this._schema(function() {
                return function(error, result) {
                    return callback(error, result);
                };
            }(this));
        };
        MojioClient.prototype.watch = function(observer, observer_callback, callback) {
            return this.request({
                method: "POST",
                resource: Observer.resource(),
                body: observer.stringify()
            }, function() {
                return function(error, result) {
                    if (error) return callback(error, null);
                    observer = new Observer(result);
                    return observer;
                };
            }(this));
        };
        MojioClient.prototype.ignore = function(observer, observer_callback, callback) {
            observer || callback("Observer required.");
            return null != observer["subject"] ? null === observer.parent ? this.signalr.unsubscribe(this.options.signalr_hub, "Unsubscribe", observer.id(), observer.subject.id(), observer_callback, callback) : this.signalr.unsubscribe(this.options.signalr_hub, "Unsubscribe", observer.id(), observer.subject.model(), observer_callback, callback) : null === observer.parent ? this.signalr.unsubscribe(this.options.signalr_hub, "Unsubscribe", observer.id(), observer.SubjectId, observer_callback, callback) : this.signalr.unsubscribe(this.options.signalr_hub, "Unsubscribe", observer.id(), observer.Subject, observer_callback, callback);
        };
        MojioClient.prototype.observe = function(subject, parent, observer_callback, callback) {
            var observer;
            null == parent && (parent = null);
            if (null === parent) {
                observer = new Observer({
                    ObserverType: "Generic",
                    Status: "Approved",
                    Name: "Test" + Math.random(),
                    Subject: subject.model(),
                    SubjectId: subject.id(),
                    Transports: "SignalR"
                });
                return this.request({
                    method: "PUT",
                    resource: Observer.resource(),
                    body: observer.stringify()
                }, function(_this) {
                    return function(error, result) {
                        if (error) return callback(error, null);
                        observer = new Observer(result);
                        return _this.signalr.subscribe(_this.options.signalr_hub, "Subscribe", observer.id(), observer.SubjectId, observer_callback, function() {
                            return callback(null, observer);
                        });
                    };
                }(this));
            }
            observer = new Observer({
                ObserverType: "Generic",
                Status: "Approved",
                Name: "Test" + Math.random(),
                Subject: subject.model(),
                Parent: parent.model(),
                ParentId: parent.id(),
                Transports: "SignalR"
            });
            return this.request({
                method: "PUT",
                resource: Observer.resource(),
                body: observer.stringify()
            }, function(_this) {
                return function(error, result) {
                    if (error) return callback(error, null);
                    observer = new Observer(result);
                    return _this.signalr.subscribe(_this.options.signalr_hub, "Subscribe", observer.id(), subject.model(), observer_callback, function() {
                        return callback(null, observer);
                    });
                };
            }(this));
        };
        MojioClient.prototype.unobserve = function(observer, subject, parent, observer_callback, callback) {
            return observer && null != subject ? null === parent ? this.signalr.unsubscribe(this.options.signalr_hub, "Unsubscribe", observer.id(), subject.id(), observer_callback, callback) : this.signalr.unsubscribe(this.options.signalr_hub, "Unsubscribe", observer.id(), subject.model(), observer_callback, callback) : callback("Observer and subject required.");
        };
        MojioClient.prototype.store = function(model, key, value, callback) {
            return model && model._id ? this.request({
                method: "PUT",
                resource: model.resource(),
                id: model.id(),
                action: "store",
                key: key,
                body: JSON.stringify(value)
            }, function() {
                return function(error, result) {
                    return error ? callback(error, null) : callback(null, result);
                };
            }(this)) : callback("Storage requires an object with a valid id.");
        };
        MojioClient.prototype.storage = function(model, key, callback) {
            return model && model._id ? this.request({
                method: "GET",
                resource: model.resource(),
                id: model.id(),
                action: "store",
                key: key
            }, function() {
                return function(error, result) {
                    return error ? callback(error, null) : callback(null, result);
                };
            }(this)) : callback("Get of storage requires an object with a valid id.");
        };
        MojioClient.prototype.unstore = function(model, key, callback) {
            return model && model._id ? this.request({
                method: "DELETE",
                resource: model.resource(),
                id: model.id(),
                action: "store",
                key: key
            }, function() {
                return function(error, result) {
                    return error ? callback(error, null) : callback(null, result);
                };
            }(this)) : callback("Storage requires an object with a valid id.");
        };
        MojioClient.prototype.getToken = function() {
            if (null != this.auth_token) return this.auth_token;
            return null;
        };
        MojioClient.prototype.getTokenId = function() {
            if (null != this.auth_token) return this.auth_token._id;
            return null;
        };
        MojioClient.prototype.getUserId = function() {
            if (null != this.auth_token) return this.auth_token.UserId;
            return null;
        };
        MojioClient.prototype.isLoggedIn = function() {
            return null !== this.getUserId();
        };
        MojioClient.prototype.getCurrentUser = function(callback) {
            if (null != this.user) callback(this.user); else {
                if (!this.isLoggedIn()) return false;
                get("users", this.getUserId()).done(function(user) {
                    if (!(null != user)) return;
                    this.getUserId() === this.user._id && (this.user = user);
                    return callback(this.user);
                });
            }
            return true;
        };
        return MojioClient;
    }();
}).call(this);